...
맥 전문가판은 휴대폰 요약판과 다르게 아래 항목을 함께 보여줍니다.

- 캔들 차트, MA5/20/60, 볼린저밴드, VWAP, 지지/저항
- RSI14, MACD, ATR14, 추세 정렬 점수
- 진입 후보가, 손절 기준, 1차/2차 목표가, 손익비
- 외국인/기관 수급, 거래량, 글로벌 지표를 반영한 체크리스트
- KIS 데이터가 과거 차트와 10배 단위 차이로 들어오면 자동 가격 보정

맥 전문가판에는 로컬 전용 모의투자 계좌도 있습니다.

- 기본 가상 예수금은 1,000만원이며 화면에서 다시 설정할 수 있습니다.
- 앱 분석 목록에 올라온 종목만 현시세 기준으로 모의매수할 수 있습니다.
- 보유 종목은 현재가 기준 평가손익, 수익률, 현재 앱 신호를 같이 표시합니다.
- 매도하면 실현손익과 매도 승률이 기록되어 앱 신호의 신뢰도를 점검할 수 있습니다.
- 모의투자 기록은 `data/paper_portfolio.json`에만 저장되고 GitHub에는 올라가지 않습니다.
...
PDF의 바닥매집주 조건을 별도 점수로 계산합니다.

- 60월선 대체 장기선 근처 지지
- 박스권 하단 지지권 근접
- 하단 지지 반복 확인
- 거래량 건조 후 대기권 또는 하단권 거래량 유입
- 박스 상단 돌파 시도
- 당일 급등은 분할매도/추격주의로 표시

조건에 걸리는 종목이 있으면 `바닥매집` 태그가 붙고, PC 대시보드는 첫 후보 상세창을 자동으로 띄웁니다. 휴대폰 화면은 상단에 `바닥매집 알림` 카드를 자동 표시하고 해당 종목 카드를 펼쳐 보여줍니다.
